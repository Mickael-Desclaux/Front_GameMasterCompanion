						...
						<div className="flex flex-col items-center border-2 border-gray-200 rounded-lg p-4">
							...
							<div className="flex flex-col w-full">
								<div className="flex flex-row gap-2">
									<UtilStatDisplay
										label="CA"
										children={character.characterArmor}
									/>
									<UtilStatDisplay
										label="Initiative"
										children={
											character.characterInitiative >= 0
												? `+${character.characterInitiative}`
												: `-${character.characterInitiative}`
										}
									/>
									<UtilStatDisplay
										label="Vitesse"
										children={`${character.characterSpeed}m`}
									/>
								</div>
								{/*stats*/}
								<div className="flex flex-row gap-2 items-start">
									<div className="flex flex-col w-1/3">
										<ul className="flex flex-col gap-8 p-4 text-sm">
											<StatDisplay
												label={"FOR"}
												value={character.characterStats.strength}
											/>
											<StatDisplay
												label={"DEX"}
												value={character.characterStats.dexterity}
											/>
											<StatDisplay
												label={"CON"}
												value={character.characterStats.constitution}
											/>
											<StatDisplay
												label={"INT"}
												value={character.characterStats.intelligence}
											/>
											<StatDisplay
												label={"SAG"}
												value={character.characterStats.wisdom}
											/>
											<StatDisplay
												label={"CHA"}
												value={character.characterStats.charisma}
											/>
										</ul>
									</div>
									...
									<div className="flex flex-col w-2/3 p-4">
										<ul className="flex flex-col gap-2">
											<li className="w-full border-2 rounded-sm">
												<div className="flex flex-row items-center gap-2 w-full p-4 font-bold">
													<span className="px-1 py-0.5 bg-gray-200 w-8 text-center rounded-sm">
														{character.characterInspirations}
													</span>
											...
											<li className="w-full border-2 rounded-sm">
												<div className="flex flex-row items-center gap-2 w-full p-4 font-bold">
													<span className="px-1 py-0.5 bg-gray-200 w-8 text-center rounded-sm">
														+{addProficiencyBonus(character.characterLevel)}
													</span>
													<span className="text-center w-full">
														Bonus de maîtrise
													</span>
											...
											<SavingThrowsList
												statProficiencies={
													character.characterStatsProficiencies as Proficiency[]
												}
											...
											<SkillsList
												skillsProficiencies={
													character.characterSkillsProficiencies as Skill[]
												}
												stat={character.characterStats}
												level={character.characterLevel}
											/>
								...
								<div className="flex flex-col items-center gap-2">
									<div className="flex flex-row w-full border-2 border-gray-200 p-4 rounded-lg gap-4 items-center font-bold">
										<div className="px-1 py-0.5 bg-gray-200 w-8 text-center rounded-sm">
											{10 +
												Number(
													computeStatBonus(character.characterStats.wisdom)
												)}
										</div>
										<div>Sagesse (Perception) Passive</div>
									</div>
									<div className="flex flex-col w-full border-2 p-4 rounded-lg gap-2">
										<ul>
											<li className="pb-1 font-bold">Langues:</li>
											{character.characterLanguages.map(
...
function SkillsList({ skillsProficiencies, stat, level }: SkillsListProps) {
	return (
		<ul className="border-2 border-gray-200 rounded-lg p-4 flex flex-col gap-2">
			<SkillStat
				skillProficiencies={skillsProficiencies}
				label="Acrobaties"
				stat={stat.dexterity}
...
function SavingThrowsList({
	...
	return (
		<ul className="border-2 border-gray-200 rounded-lg p-4 flex flex-col gap-2">
			<SavingThrowStat
				statProficiencies={statProficiencies}
				statLabel="Force"
				stat={stat.strength}
...
function SavingThrowStat({
	statProficiencies,
	statLabel,
	stat,
	proficiency,
	level,
}: SavingThrowStatProps) {
	return (
