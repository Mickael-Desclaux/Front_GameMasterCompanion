		...
		<li className="pb-4">
			<div className="flex flex-col w-full justify-center">
				<div className="font-bold flex flex-row items-center">
					<NavLink
						to={`chapter/${id}`}
						className="flex w-full flex-wrap text-balance justify-start text-lg"
					>
						{title}
					</NavLink>
			...
			<div className="flex flex-col lg:flex-row w-full h-screen">
				<div className="flex flex-col w-full lg:w-1/3">
					<div className="h-full m-2 border-2 border-gray-200 rounded-lg p-4 overflow-auto">
						<h2 className="pb-4 underline text-2xl font-bold text-center">
							Chapitres
						</h2>
						<ul>
				...
				<div className="w-full lg:w-2/3">
					<div className="h-full w-full flex flex-col">
						<div className="border-2 border-gray-200 rounded-lg m-2 p-4">
							<NavLink to={`/campaign/${campaignId}/players`}>
								<h2 className="pb-4 underline text-2xl font-bold text-center">
									Joueurs
								</h2>
							</NavLink>
								...
								{campaignData.characters.map((character, index) => (
									<div key={index}>
										<PlayerStatus
											...
											playerRace={character.characterRace}
											playerLevel={character.characterLevel}
											playerIcon={character.characterIcon}
											playerCurrentHP={character.characterCurrentHealth}
											playerMaxHP={character.characterMaxHealth}
										/>
									</div>
								))}
							</div>
						...
						<div className="border-2 border-gray-200 rounded-lg m-2 p-4 overflow-auto">
							<h2 className="font-bold text-2xl underline text-center">
								Notes
							</h2>
							<ul className="p-2">
								{campaignData.notes.map((note) => (
									<li key={note.id} className="pb-2">
										<div className="font-bold underline">{note.title}</div>
										<div className="text-sm text-balance">{note.content}</div>
									</li>
								))}
							</ul>
						</div>
